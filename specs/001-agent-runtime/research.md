...
## 4. Context compression — retrieved context is a seam; conversation history is not

Two different things were being called "compression", and collapsing them is what left the one real gap.

- **Decision (retrieved context — deferred)**: Leave a pre-send seam in the gateway client where a compression step over `context` can be inserted, but do not implement it now.
- **Rationale**: Compressing retrieved chunks trades recall for tokens, and the trade is only measurable once real prompts and a real eval set exist. Building the seam is nearly free; building the policy early means tuning it against imagined traffic. `context` is already bounded by `assemble`'s token trim, so deferring costs nothing but tokens.
- **Decision (conversation history — built in Phase 1)**: `history` is bounded by an explicit `history_token_budget` and compacted keep-first / keep-last / summarize-the-middle at the graph entrypoint (AR-015a, [agent-graph.md](./contracts/agent-graph.md#conversation-context-budget--bounding-history-ar-015a)).
- **Why this one could not be deferred the same way**: `history` was the **only unbounded input in `AgentState`** — every other input is capped by construction (`candidates` by `top_k`, `context` by the trim, tool results by `max_result_bytes`). An unbounded input is not a tuning question, it is three defects waiting: a provider context-length refusal that no reliability row accounted for, unbounded prompt cost across the four gateway call sites a semantic run makes, and — worst — a silent quality cliff the moment some layer starts truncating from the front. "Measure before optimizing" applies to *how much* to keep; it does not apply to *whether there is a ceiling*.
- **Alternatives considered**: (a) Shipping a `context` compressor immediately — rejected: unmeasurable, and a bad policy silently degrades answer quality in a way the smoke test cannot see. (b) Truncating `history` from the front instead of summarizing — rejected: the opening turns carry the framing the rest of the session is relative to, so front-truncation is the one strategy that reliably loses what the conversation is *about*. (c) Compacting inside each node that reads `history` — rejected: `route`, `rewrite`, and `generate` would each see a different history in one run, making the debug panel incoherent.

## 5. Agent memory scoping — visibility *and* content trust

- **Decision**: Memory is stamped with tenant and principal on **write**, and re-filtered against **current** clearance on every **read**. A memory written when a principal had broader access is not readable after a demotion. **Separately and additionally**, a recalled memory is delimited as **untrusted content** exactly like a retrieved chunk, and no backend may write memories autonomously.
...
- **Why the second half is not implied by the first**, which is the trap: clearance answers *may this principal see this text*. It says nothing about *is this text trying to issue an instruction*, and the two failures are independent. Memory is the only prompt input that is both **persistent** and **cross-session** — `guard` screens the current query, so a poisoned sentence that entered on turn 1 is re-injected on every later turn without ever being re-screened. A perfectly clearance-correct memory can carry a perfectly effective injection. Both properties are required and neither substitutes for the other.
- **Consequence for the write path**: whatever writes memory decides what is replayed into every future prompt, which makes it a privileged surface. Off-the-shelf memory backends commonly extract and persist facts from turn text by default — that is precisely the autonomous write §13 excludes from Phase 1, arriving through a config default rather than a design decision. Such a backend is bound with extraction **disabled**, or wrapped so every write is an explicit audited call. Asserted by `MemoryServiceContract`, not left to the backend's defaults.
- **The third property: a memory must be removable.** Stamping and re-filtering govern *who sees* a memory; neither ends its life. Two cases need that and neither is exotic. A memory identified as carrying an injection is, by this section's own argument, replayed into every future prompt until something deletes it — so a runtime that can detect one and not remove it has diagnosed the problem and stopped. And a principal who leaves or asks to be forgotten leaves durable personal content here, in the one store the runtime owns that is not the host's corpus. `MemoryService` therefore carries a scoped, audited `delete` plus a **retention horizon enforced at recall**, not only by a sweep — a guarantee that holds between cleanup runs rather than after them ([the `MemoryService` port](./contracts/agent-deps.md#memoryservice-streamwriter-checkpointer-bus)).
- **Alternatives considered**: (a) Trusting a prior turn's snapshot — rejected: it converts a clearance change into a permanent leak. (b) Treating clearance re-filtering as sufficient — rejected for the reason above: it is a visibility control being asked to do a content-trust job. (c) Injection-scanning memories at write time instead of delimiting at read time — rejected: detection is a filter and framing is structural, and the repo's standing preference (§1) is structural defenses over detective ones. (d) Leaving deletion to each backend's own admin surface — rejected: it makes erasure a property of which backend a deployment happened to bind, when the whole point of the port is that a capability the runtime depends on is stated in the protocol and checked by the suite. (e) Soft-deleting with a tombstone the backend still recalls — rejected: a memory that still reaches a prompt has not been deleted, whatever the row says.
...
## 14. The capability budget — at most two legs of the trifecta without a human

- **Decision**: A run's exposure is described by three legs — **(A)** access to private data, **(B)** exposure to untrusted content, **(C)** the ability to mutate state or reach outside the corpus. Any **one** unsupervised run may hold at most **two**. A configuration that would hold all three MUST route the (C) step through the human gate, and every tool — from **any** source, including a remote MCP catalog — MUST **declare** which legs it adds.
- **Rationale**: Phase 1 already satisfies this, and that is precisely the problem worth naming. The interactive form holds A and B and has no action tool at all; the durable form adds C and puts a human in front of it. The property is real and it was arrived at by careful case-by-case reasoning — which is a method that works exactly as long as the person adding the next tool repeats it. The Phase-2 list is where it stops working: broader writes, `write_memory`, artifact writes, and the sandbox tools each add leg C to a runtime that permanently holds A and B, and each will be argued on its own merits by whoever writes it. Making the budget explicit converts "we thought about it each time" into a question a reviewer must see answered, which is the same move `check-vocabulary.sh` makes for prose and the import scan makes for port discipline: **mechanical instead of remembered**.
- **The gap this exposed, which is not hypothetical**: `tools.kind: mcp_client` points the unmodified graph at a **remote** MCP server as its tool source, by config alone. Every existing statement about that path governs the **access floor** — the remote server sets its own session context, applies its own allowlist, writes its own audit — and is correct. None of them governs **what those tools do**. A remote catalog exposing a `send_message` or `create_ticket` tool hands leg C to a run holding A and B, with no `human_gate` in the path, because the gate is driven by `pending_approval` that only the two Phase-1 action nodes ever set. The manifest invariant "narrows, never widens" was stated entirely about *what a principal may see* and is silent about *what the agent may do* — so a config-only change can widen the second while provably not touching the first.
- **Consequence**: `ToolSpec` carries a declared capability (`read_only` \| `mutating` \| `outward`), the wrapper refuses to dispatch a `mutating`/`outward` tool that is not gated and not in the durable form, and an undeclared capability is treated as `outward` — fail closed, because the dangerous default is the one an unmaintained third-party catalog will hit. This is enforcement in the shared wrapper, which is where §7 already puts every other enforcement point.
- **Alternatives considered**: (a) Trusting the remote server to gate its own mutating tools — rejected: it is the same category error as trusting a manifest to be a policy. The remote server owns *authorization*, which is who may call; the gate is *oversight*, which is whether a human saw it, and that is this runtime's obligation to the principal it is acting for. (b) A guardrail model classifying tool intent at dispatch — rejected per §1's standing preference: detection is a filter, declaration is structural, and an adaptive attacker beats the filter. (c) Banning `mcp_client` outright — rejected: the decoupled topology (§12) is a real capability, and the fix is a declaration requirement, not the loss of a deployment shape.
