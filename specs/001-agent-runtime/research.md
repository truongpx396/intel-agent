...
- **Alternatives considered**: (a) Trusting a prior turn's snapshot — rejected: it converts a clearance change into a permanent leak. (b) Treating clearance re-filtering as sufficient — rejected for the reason above: it is a visibility control being asked to do a content-trust job. (c) Injection-scanning memories at write time instead of delimiting at read time — rejected: detection is a filter and framing is structural, and the repo's standing preference (§1) is structural defenses over detective ones.
