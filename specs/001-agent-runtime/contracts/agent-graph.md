...
**Plan**: [../plan.md](../plan.md) | **Ports**: [agent-deps.md](./agent-deps.md) | **Host obligations**: [host-integration.md](./host-integration.md) | **Tools**: [mcp-tools.md](./mcp-tools.md)

The internal contract for the graph (`src/intel_agent/graph/`). Where [agent-deps.md](./agent-deps.md) declares *what the runtime needs from the outside*, this declares *what happens inside*: the shared state schema, each node's read/write keys, how nodes receive their dependencies, how state is checkpointed and resumed, and how tokens are streamed out.

It exists so the "additive node, no refactor" claims are **verifiable against a written state schema**, and so the graph can be reasoned about, tested per node, and run inside any host (see [Extraction checklist](#extraction-checklist)).

> **Reading conventions.** Normative prose uses the domain-agnostic vocabulary — `tenant`, `principal`, `agent_role`, `claims`. Where a concrete example clarifies, it is drawn from the **reference host** ([aisat-intel](https://github.com/truongpx396/aisat-intel), the deployment this runtime was extracted from) and is marked *reference host:*. Such an example is an illustration, **never a requirement** — nothing here may depend on that host's field names, role labels, or clearance model. If you are building against this contract, you can ignore every reference-host aside and lose nothing normative.
...
| `agent_run` row | none | one row (SC-A06, [data-model.md](../data-model.md) Long-Horizon Task Run) |
| Iteration | single forward pass | **bounded** iterative re-entry, checkpoint after every node, `credits_cap`-governed |
| Human-gate (Phase 1) | disabled (no durable approval home) | **enabled** — `human_gate` may `interrupt()` before an index-mutating / outward-reaching step, `status='paused'` (AR-005, [approval-ports.md](./approval-ports.md)) |
...
class SecurityCtx(TypedDict):        # stamped ONCE by the host's trusted layer; never mutated by a node
    # --- domain-agnostic core: the graph/runtime reads ONLY these opaque fields ---
    tenant: str                      # opaque tenant key
    principal: str                   # opaque principal key
    agent_role: str                  # host-defined role label the allowlist is keyed on; the runtime
                                     #   never interprets its value, only passes it to the policy wrapper
    allowed_tools: list[str]         # from the manifest; never hardcoded
    trace_id: str
    stream_id: str                   # streaming key; the graph never touches a stream backend with it
    # --- domain claims: OPAQUE to the runtime; read ONLY by injected deps, never by a node ---
    claims: dict                     # host-specific authorization facts, flowed through ctx into deps.
                                     #   A support bot might stamp {"customer_id": …}; a course tool
                                     #   {"course_ids": [...]}; the reference host stamps a numeric
                                     #   clearance level. NO graph node dereferences claims — only deps do.
...
class AgentState(TypedDict, total=False):
    ...
    step_fingerprint: str | None     # hash of (node, rewritten_query, tool calls added this step) —
                                     #   the no-forward-progress signal; bounded re-entry is not progress
    no_progress_count: int           # consecutive identical fingerprints; ≤ max_no_progress_steps
    ...
    # --- agent actions (Phase 1, durable form only; AR-004) ---
    ...
    # --- clarification (AR-011, interactive AND durable — it pauses nothing) ---
    ...
    clarifies: dict | None           # {id, option_id?} echoed from the follow-up query, so eval can measure
                                     # whether asking actually improved the answer (SC-A09).

    # --- scoped questions & attachments (AR-009/AR-009) ---
    doc_ids: list[str] | None        # when set, `retrieve` searches ONLY these documents — a NARROWING
                                     ...
                                     # host before the graph runs, so the node never sees an unauthorized id.
    ...
    image_refs: list[dict]           # [{document_id, s3_key, mime}] images the turn must actually LOOK at
                                     # (AR-010); `generate` routes to the `vision` alias when non-empty.

    # --- observability (SC-A05) ---
    debug: dict                      # per-node trace fragments → the `rag_retrieval` section payload (sse-events.md)
```

**Reducer semantics.** Only `tool_calls` uses a non-default reducer (`operator.add`, append-only) because a long-horizon run accrues tool calls across re-entries and none may be lost for audit (AR-019). Every other key uses last-writer-wins (the default): each key has exactly one owning node, so there is no write contention. `intent`/`model_alias` are written only by `route` and are treated as immutable — enforced by convention and a unit test, not by a reducer.

### Domain-agnostic security context (reusability seam)

`SecurityCtx` is split into two tiers so the graph lifts into another host unchanged — the runtime-side counterpart to the way [the `Policy` port](./agent-deps.md#policy) factors the *decision engine* out of any particular access model:

- **The domain-agnostic core** (`tenant`, `principal`, `agent_role`, `allowed_tools`, `trace_id`, `stream_id`) is all the *runtime* reads. `tenant`/`principal` are opaque keys the host binds from its own identities; `agent_role` is an opaque label the `allowed_tools` allowlist is keyed on. No node interprets their *values*.
- **The `claims` bag** carries every host-specific authorization fact — `customer_id`, `course_ids`, an RBAC role set, a numeric clearance, whatever that access model needs. **The runtime never dereferences `claims`.** It flows `ctx` (claims included) into the injected deps (`RetrievalService`, `MemoryService`, `ToolRegistry`, `Policy`), which are the *only* code that reads a domain claim and lowers it into a store predicate.

> *Reference host:* stamps `{"effective_access_level": int}` — a 1–5 clearance ladder evaluated at read time — and lowers it to Postgres RLS GUCs plus a vector payload pre-filter. That is one access model among many, and the graph cannot tell which one is in use.

This is what makes the access model swappable without touching the graph: replacing one `Policy` implementation ([the `Policy` port](./agent-deps.md#policy)) with a per-customer or role×course policy changes only the deps that read `claims`, not the nodes. Every host-specific authorization value lives at `ctx.claims[<key>]`, and the host stamps it when it builds `ctx`. Likewise `ctx.tenant`/`ctx.principal` are bound from the host's own identities. The concrete store predicates named in [mcp-tools.md](./mcp-tools.md) and [data-model.md](../data-model.md) are those bindings evaluated **inside the deps**, against real payload keys and session variables — they are not fields the graph reads, and a backend's field names stay concrete (renaming one is a schema change, not a runtime abstraction).

> *Reference host:* stamps `effective_access_level` into `claims` and lowers it to `workspace_id == ctx AND access_level <= effective_access_level`.

### Observability fragments — what each node owes the debug panel (AR-017, SC-A05)

`state["debug"]` is assembled by the nodes that own each fact, then rendered as the `rag_retrieval`, `grounding`, and `cost` sections ([the `StreamWriter` port](./agent-deps.md#memoryservice-streamwriter-checkpointer-bus)). Ownership is exclusive — one writer per fragment, matching the last-writer-wins reducer.
...
| `cost.calls[]` | every gateway call site | one entry per call — `rewrite`, `rerank`, `generate`, `vision`, with tokens, credits, duration, fallback hops, and the **resolved model** the alias mapped to ([the `LLMGatewayClient` port](./agent-deps.md#llmgatewayclient)) |
...
**And the count leaves the panel, because a panel nobody opened reports nothing.** The debug fragment is the *explanation*, rendered by a host, for one member, on request. The bare **count** of unsupported claims is a different thing: it is the one grounding fact a consumer needs in order to render an answer honestly, and it belongs in the event vocabulary as a structural field (`run_finished.ungrounded_claims`, [stream-events.md](./stream-events.md)) for exactly the reason `answer_generated: false` is structural — a consumer must be able to tell a fully-grounded answer from one carrying three unsupported claims **without reading the prose**, and a sentence the model was asked to write is not a protocol. Two consequences worth stating: the count is emitted whether or not a host renders a panel at all, and it makes grounding assertable from the published vocabulary, which is what lets the eval gate ([Evaluation](../spec.md)) measure it from the same golden fixtures a host tests against rather than by reaching into runtime internals. Phase 1 **reports**; acting on the number — regenerating, abstaining — is the Phase-2 `grade_answer` seam, and reporting it now is what gives that seam a baseline to be judged against.
...
### Node telemetry — logs and metrics, which the debug panel is not (AR-018, AR-020)
...
| `agent_degradation_total` | counter | `node`, `reason` (`rerank_fallback`, `memory_unavailable`, `rewrite_passthrough`, `vision_failed`, `history_compacted`, `deadline_partial`, `budget_partial`, …) |
...
> **Scope boundary.** This contract fixes what the graph *emits*. Dashboards, alert rules, and published latency/throughput SLOs are the Phase-4 load-and-SLO item ([draft-plan.md §9](https://github.com/truongpx396/aisat-intel/blob/main/specs/draft-plan.md)) — but they cannot be built retroactively over data nobody recorded, which is why emission is Phase 1.

### Clarification — asking instead of guessing (AR-011)
...
### Scoped retrieval, attachments, and images (AR-009–AR-010)
...
>
> **And a consequence for the removal path, which follows from the same sentence.** "Re-injected on every subsequent turn until something deletes it" names a mechanism this contract owes and, until [the `MemoryService` port](./agent-deps.md#memoryservice-streamwriter-checkpointer-bus) grew a `delete`, did not have. Clearance re-filtering hides a memory from a demoted principal; it does not remove one, so a memory found to carry an injection stayed on the replay path for the life of the account with no supported way to take it off. `delete` (scoped by `ctx`, audited through `Recorder`, returning a count) plus a retention horizon enforced **at recall** is that mechanism — the removal half of the untrusted-memory argument, not a compliance feature bolted alongside it.
...
- **The caption still does the finding.** Ingestion-time captions (`fast`, AR-021) remain the retrieval surface — they are how an image is located by text search at all. `vision` answers about what the caption did not anticipate. Removing either one breaks a different half of the feature.
- **Fail closed, and say so.** If the `vision` call fails past its one fallback hop, `generate` degrades to a caption-grounded answer that **states the image was not examined**. Degrading silently — answering from a summary while the member believes the image was read — is the specific failure this rule forbids (AR-010). Recorded in `debug` and surfaced as `status:"degraded"` on the `agent_step`, exactly like the `rerank`→RRF fallback.
...
- **Checkpoint loss is explicit, never silent.** If the pointer exists but the Redis checkpoint is gone (AOF gap / failover before persist), the run transitions to `failed` with `error='checkpoint_lost'` — it does **not** restart from scratch, because a restart would re-spend credits already settled (SC-A06). Interactive runs carry no `agent_run` row and are simply re-dispatched by JetStream redelivery (a lost interactive checkpoint is cosmetic).

### A checkpoint written by one build MUST NOT resume silently into another (AR-016a)

`checkpoint_lost` covers the checkpoint that is *gone*. The likelier production failure is the checkpoint that is *present and stale*: a durable run parks at `human_gate` — which pauses **indefinitely**, by design, and may wait days for an approver ([the reliability table](#per-node-reliability-policy)) — the service is deployed twice in the meantime, and the resuming worker loads a checkpoint written by a build whose `AgentState` schema, node set, or edge order no longer exists. LangGraph will resume it. Nothing in the two rules above notices.

The failure modes are not hypothetical and they are silent in different ways: a key renamed between builds resumes as a **missing** key rather than an error; a node removed from the topology leaves a checkpoint pointing at a `node` name the new graph cannot schedule; a node **inserted** before the pause point means the resumed run skips a step it would have executed had it started under the new build. Each produces a run that completes and looks clean.

So the pointer carries the build that wrote it, and resume compares before it continues:

- **`agent_run.state` records `graph_version` and `state_schema_version`** alongside the checkpoint coordinates ([data-model.md](../data-model.md)). `graph_version` is the deployed build SHA — the same value already in every telemetry record, now durable rather than only observed. `state_schema_version` is an integer bumped by any change to the `AgentState` keys or the node set; it is what makes compatibility a *declared* property instead of one inferred from a SHA that changes on every deploy, including the ones that touch nothing here.
- **Equal schema versions resume.** A deploy that changes prompts, a backend, or a timeout leaves `state_schema_version` alone, so ordinary deploys — the overwhelming majority — resume exactly as they do today. The SHA differing is recorded, not fatal.
- **A differing schema version fails the run** `failed('checkpoint_incompatible')`, with both versions in `debug`. Same discipline as `checkpoint_lost` and for the same reason: a run that cannot be resumed correctly must say so, because the alternative is a wrong answer that costs the same as a right one. Spend already settled stays settled; the approver's decision is preserved on the `approval_request` so resolving a gate is never lost work, only the run behind it.
- **The interactive form is exempt**, having no durable checkpoint to disagree with. A lost or stale interactive checkpoint is cosmetic, as above.

> **This is a compatibility rule, not a deployment strategy.** Running old and new workers side by side until in-flight runs drain (the rainbow pattern) is a *host* operational concern and stays out of this contract — the runtime is a stateless worker and does not schedule its own replicas. What the runtime owes is the mechanism that makes such a strategy checkable and its absence loud: a version stamped at write time and compared at read time. A host with no drain strategy gets explicit failures instead of silent ones, which is the honest floor. **Widening the gate is the migration path**: when a schema change is genuinely backward-compatible, the way to say so is to leave `state_schema_version` unchanged — deliberately, in the PR that makes the change — never to relax the comparison.

## Human-in-the-loop: the `human_gate` node (durable form)

The idiomatic LangGraph HITL primitive — **`interrupt()` + `Command(resume=…)`** — is built into the single `StateGraph` in Phase 1 and is the mechanism behind AR-005 and the reusable [approval-ports.md](./approval-ports.md) `HumanGate`. It exists **only in the durable (`long_horizon`) compiled form** because a pause must be recoverable; the interactive form has no durable approval home and disables it, so an interactive query can never silently pause.
...
- **Suspend → resume flow.** `interrupt()` writes a checkpoint and returns control; the run's `agent_run.status` becomes `paused` and the host emits an `approval_request` SSE event ([the `StreamWriter` port](./agent-deps.md#memoryservice-streamwriter-checkpointer-bus)). Resolving via `POST /approvals/{id}/resolve` publishes `agent.resume.<tenant>` ([the `Bus` port](./agent-deps.md#memoryservice-streamwriter-checkpointer-bus)); the worker loads the paused thread's checkpoint (`agent_run.state.thread_id`) and resumes with `Command(resume=decision)`.
- **Exactly-once, no re-spend.** Resume re-enters *at* the gate checkpoint, so nodes settled before the gate are not re-run and their spend is never repeated (parity with the `checkpoint_lost` rule and SC-A06). A redelivered `agent.resume.<tenant>` or a second approve is a no-op — the thread is already past the gate.
- **Decision provenance.** `approval_decision` is injected by `Command(resume)` from the human's resolution — it is **never** derived from tool/document output (research §5, AR-002). This is why a poisoned page can at worst influence a *draft the human reviews*.
- **Phase-1 callers of the gate.** The durable form has two always-on state-changing/reach steps in Phase 1 (AR-004), each routing through `human_gate`:
...
### Run-level budgets — the boundaries above the per-node timeouts (AR-015)
...
| **Per-run credits** | `run_credits_cap` | `credits_cap` | `agent_policies` / `agent_run` | halt (AR-014 behavior), degrading past `assemble` exactly as a deadline does |
| **No forward progress** | n/a — single forward pass | `max_no_progress_steps` (default 3) | `agent_policies` | `error='no_progress'` |
| **Daily tokens** | `token_budget_day` | `token_budget_day` | `agent_policies` | admission refused before the run starts |

Seven properties make these boundaries honest rather than decorative:
...
- **A breach is settled, recorded, and terminal — never retried.** Spend already incurred is settled (a breach is not a refund event), `agent_budget_exhausted_total{boundary}` increments — the label's closed vocabulary is exactly `deadline` \| `steps` \| `context_window` \| `credits_cap` \| `no_progress` \| `token_budget_day` — the reason is written to `debug` so the panel shows *why* the run stopped, and the run ends `failed` or `degraded` per the rule below. If tokens have already streamed, the post-first-token rule from `generate` applies unchanged: settle the partial, do not re-run.
- **Cost bounds the interactive run too, and it is the boundary that generalizes.** The wall-clock deadline is a *latency* control that only incidentally bounds spend, and it bounds it badly: how much a run can buy in 120 seconds is a property of the provider's throughput on the day, not of the run. A `semantic` pass issues [4–5 gateway calls](#node-identity) and `generate` may escalate `max_output_tokens` 4× on a truncated finish ([the `LLMGatewayClient` port](./agent-deps.md#llmgatewayclient)), so the interactive path's real spend ceiling was, until this row existed, "whatever the daily budget still had left" — one pathological query could consume a day's allowance inside the deadline and every boundary in this table would report a clean run. `run_credits_cap` is evaluated at the same node boundaries as the deadline, from the same settled-spend figure the `Meter` already tracks, and degrades past `assemble` under the identical rule: a run that has bought its context should answer from it rather than discard four settled calls. **A step count is not a substitute** — the same step costs a different amount on `fast` and on `smart`, so a cap tuned on one alias is wrong on the other, which is exactly why the step cap is a backstop here and never the primary boundary.
- **Bounded re-entry is not the same as forward progress.** `max_steps` bounds how *many* times a durable run may traverse the retrieval/generation nodes; it says nothing about whether the traversals differ. A run that re-enters with the same `rewritten_query` against the same tool arguments will do so 60 times, pay the amplified call rate 60 times, and settle — the cap fires, the run ends `step_cap_exceeded`, and the spend is indistinguishable from a run that was genuinely working. The runtime therefore fingerprints each step as `(node, rewritten_query, tool_calls added this step)`, hashed; `max_no_progress_steps` consecutive identical fingerprints end the run `failed('no_progress')`. Three properties keep this cheap and honest: the fingerprint is computed from state the step already wrote (no extra model call), an *empty* retrieval is progress the first time and repetition thereafter (`source_count == 0` remains a valid answer state, [not a failure](#per-node-reliability-policy)), and the counter resets on any differing fingerprint — a run that oscillates between two distinct states is bounded by `max_steps`, which is the boundary that fits it. Detecting a loop early is a cost control; the caps remain the correctness backstop, and neither replaces the other.
...
**The credits cap degrades on the same boundary, for the same reason.** A `run_credits_cap` / `credits_cap` breach at or after `assemble` returns an answer under the remaining allowance, or citations under the not-generated marker when there is none — `error='credits_exhausted'` where it fails, `degraded{reason:'budget_partial'}` where it degrades. The two boundaries measure different things (time and money) and are set independently, but the salvage argument does not distinguish them: work already bought is bought either way, and the run became able to answer at the same node. The boundaries that **never** degrade are `context_window_exceeded` (the prompt does not fit, so there is nothing to generate *from*) and `no_progress` (the run demonstrated it was not converging, so a partial is not a partial *of* anything).
...
4. **A stream adapter** for the host's transport. Any host-specific one (*reference host:* Redis pub/sub) stays **behind** this boundary — the graph must stream green with no stream backend bound.
...
- **Domain-agnostic ctx** (reusability seam): a static check (AST/import scan over `nodes/`) asserts no node module dereferences `ctx["claims"]` or any host-specific claim key (*reference host:* `effective_access_level`) — only `deps.*` may. Complementarily, the graph runs green end-to-end on a `ctx` whose `claims` bag holds an **opaque test-only key** (with fake `RetrievalService`/`ToolRegistry`), proving the runtime never depends on a specific claim shape — the graph half of the extraction guarantee, in the same "prove reuse with a second fixture" spirit as the `PricerContract`/`HashChainContract` tests.
- **Checkpoint pointer**: after a mid-run crash, `agent_run.state.thread_id` locates a Redis checkpoint whose `node` matches the last completed node; a missing checkpoint transitions the run to `failed('checkpoint_lost')`, never a silent restart (the host's ledger-exactness criterion, SC-A06).
- **Checkpoint version compatibility** (AR-016a): a run paused at `human_gate` under `state_schema_version` N and resumed by a worker declaring N+1 ends `failed('checkpoint_incompatible')` with both versions in `debug` — it does **not** resume into the new topology. The same run resumed by a worker declaring N but a **different** `graph_version` resumes normally, the differing SHA recorded and not fatal; this is the assertion that keeps the rule from making every ordinary deploy a run-killer. A third case asserts the write half: the pointer written at pause time carries both versions, so a resume that cannot compare them fails closed rather than assuming a match.
...
- **Per-run cost cap**: an **interactive** run whose settled spend crosses `run_credits_cap` **before `assemble`** ends `failed('credits_exhausted')` at a node boundary with `agent_budget_exhausted_total{boundary="credits_cap"}` incremented; one crossing it **at or after `assemble`** finishes `outcome='degraded'` with `degraded{reason:'budget_partial'}` under the same never-`ok`, never-dressed-up rule as the deadline. Asserted with a scripted `FakeGateway` that returns a usage figure large enough to cross the cap at a chosen call site, so the boundary is exercised at each of `route`, `rewrite`, `rerank`, and `generate` rather than only at the cheapest one. A run under the cap is unaffected, and the cap is read from the manifest — a deployment that omits it fails manifest validation closed rather than running uncapped.
- **No forward progress** (durable form): a run scripted to re-enter with an identical `(node, rewritten_query, tool_calls)` fingerprint ends `failed('no_progress')` after `max_no_progress_steps` consecutive repeats — **before** `max_steps` would have fired, which is the point — with `agent_budget_exhausted_total{boundary="no_progress"}` incremented and spend settled. Two negative assertions carry equal weight: a run whose fingerprint differs on any step has its counter **reset** and runs to `max_steps` or completion, and a run that legitimately re-enters twice with `source_count == 0` but a **changed** `rewritten_query` is not flagged — an empty retrieval is a valid answer state and repetition, not emptiness, is the signal.
...
- **Agent action tools** (durable form, AR-004): `web_search_decide` performs **no** fetch until the per-search gate is approved (a reject writes no `web_results` and spends nothing for the fetch); `edit_note` commits the note update **only** after approval **and** an Authorizer `Permit(ActionUpdate)`+`WriteEnvelope` pass with `Clearance=min(agent,owner)` — an edit above clearance / outside workspace is denied (`not_found`), one that would raise `access_level` above the source floor is denied (`envelope_widens`), and a create attempt is refused (SC-A08). Neither tool exists in the interactive compiled form.

---

## Phase 2 (out of scope here)

> The four seams above are designed but not built in Phase 1. The **planner/executor decomposition** for genuinely multi-step long-horizon agents (beyond bounded re-entry of the RAG nodes) is a Phase-2+ decision ([draft-plan.md](https://github.com/truongpx396/aisat-intel/blob/main/specs/draft-plan.md)); if long-horizon orchestration ever outgrows LangGraph, a durable-execution engine (Temporal/River) is the escalation, not a rewrite of these nodes (research §15 alternative (b)).
